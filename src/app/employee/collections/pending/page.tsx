...
import { getPendingCollectionsAction } from '@/app/actions/collection';
...
import { Search, KeyRound, Clock, Filter } from 'lucide-react';
...
export default function PendingCollectionsPage() {
  ...
  const fetchPending = async (q?: string) => {
    setLoading(true);
    const res = await getPendingCollectionsAction(undefined, q);
    ...
    <div className="min-h-screen bg-slate-50/50 dark:bg-neutral-950 text-slate-900 dark:text-zinc-100 p-6 lg:p-10 space-y-8 transition-colors">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-slate-200/80 dark:border-zinc-800 shadow-xs">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-slate-500 dark:text-zinc-400 font-semibold">Pending Queue</span>
          </div>
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white mt-1">Pending Collections Queue</h1>
        </div>
        ...
        <form onSubmit={handleSearch} className="flex items-center space-x-2 w-full md:w-auto">
          <div className="relative flex-1 md:w-72">
            ...
            <input
              ...
              placeholder="Filter by LR #, Receiver Name/Mobile..."
      ...
      <div className="bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800 rounded-3xl overflow-hidden shadow-xs">
        <div className="p-5 border-b border-slate-200/60 dark:border-zinc-800">
          <h3 className="text-sm font-extrabold uppercase tracking-wider text-slate-900 dark:text-white">Uncollected Parcels List</h3>
        </div>

        {loading ? (
          <p className="text-xs text-slate-500 py-6 text-center">Loading pending parcels...</p>
        ) : pendingList.length === 0 ? (
          <p className="text-xs text-slate-500 py-6 text-center">No uncollected parcels matching filter criteria.</p>
        ) : (
              ...
              <tbody className="divide-y divide-slate-200/60 dark:divide-zinc-800/60 font-medium">
                {pendingList.map((b) => (
                  <tr key={b.id} className="hover:bg-slate-50/50 dark:hover:bg-zinc-800/30">
                    ...
                    <td className="p-4">
                      <span className="font-extrabold text-slate-900 dark:text-white block">{b.receiverName}</span>
                      <span className="text-[10px] text-slate-400 dark:text-zinc-500 font-mono">{b.receiverPhone}</span>
                    </td>
                    <td className="p-4">{b.senderName}</td>
                    <td className="p-4">{b.originOffice?.name || 'Origin'}</td>
                    <td className="p-4 font-extrabold text-amber-600 dark:text-amber-400">₹{b.totalAmount} ({b.paymentType})</td>
