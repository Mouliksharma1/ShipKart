...
export class MockSMSProvider {
  static providerName = 'Mock FastSMS Engine (Free / Local Dev)';
  ...
  static async send(options: ProviderSendOptions): Promise<ProviderResponse> {
    ...
    console.log(`[MOCK SMS SEND] To: ${options.to} | Message: ${options.message}`);

    await new Promise((r) => setTimeout(r, 25));

    return {
      success: true,
      providerMessageId: `mock_sms_msg_${Date.now()}_${Math.floor(Math.random() * 10000)}`,
      provider: this.providerName,
      timestamp: new Date(),
    };
  }
