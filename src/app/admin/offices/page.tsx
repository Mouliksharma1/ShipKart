      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xs text-slate-500 font-semibold">Offices</span>
          </div>
          <h1 className="text-2xl font-extrabold text-slate-900 mt-1">Branch & Office Operations</h1>
        </div>
            ...
            >
              <div className="space-y-4">
                <div className="flex items-start justify-between">
                  <div>
                    <span className="text-[10px] font-extrabold uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-700">
                      {office.officeType}
                    </span>
                    <h3 className="text-lg font-extrabold text-slate-900 mt-1.5">{office.name}</h3>
                    <p className="text-xs font-mono text-slate-400">{office.officeCode || office.code}</p>
                  </div>
                  {isArchived ? (
                ...
                {office.parentOffice && (
                  <div className="text-xs text-slate-500 bg-slate-50 p-2 rounded-xl border border-slate-100 flex items-center space-x-1.5">
                    <Building className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                    <span>Parent: <strong className="text-slate-700">{office.parentOffice.name}</strong></span>
                  </div>
                )}
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-xs">
                <span className="text-slate-500 flex items-center font-medium">
                  <Users className="w-3.5 h-3.5 mr-1 text-slate-400" /> {office._count?.users || 0} Staff
                </span>
                <Link
                  href={`/admin/offices/${office.id}`}
                  className="font-bold text-orange-600 hover:text-orange-700 flex items-center"
                >
                  Manage Office &rarr;
                </Link>
              </div>
